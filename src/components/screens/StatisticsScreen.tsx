...
export const StatisticsScreen: React.FC<StatisticsScreenProps> = ({
  ...
  const monthlyData = [
    { name: isId ? 'Mei' : 'May', Income: 11000000, Expense: 6500000 },
    { name: 'Jun', Income: 12000000, Expense: 7200000 },
    { name: 'Jul', Income: 14500000, Expense: 8100000 },
    { name: isId ? 'Agu' : 'Aug', Income: totalIncome || 12500000, Expense: totalExpense || 2965000 }
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`text-xl font-black tracking-tight ${isDark ? 'text-white' : 'text-slate-900'}`}>{t("Statistik & Analisis")}</h2>
          <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{t("Visualisasi arus kas & pola pengeluaran")}</p>
        </div>
      ...
      <div className="p-5 bg-gradient-to-r from-[#0B1220] via-[#1E1B4B] to-[#6C4CF5] text-white rounded-[24px] shadow-xl border border-white/10 space-y-3">
        ...
        <div className="flex items-baseline space-x-2">
          ...
          <span className="text-xs text-indigo-200">/ 100 {t("poin")}</span>
        </div>

        <p className="text-[11px] text-slate-200 leading-snug">
          {t("Arus kas Anda berada pada kondisi sangat sehat dengan rasio tabungan di atas 30%.")}
        </p>
      </div>
      ...
      <div className={`p-5 border rounded-[24px] shadow-xs space-y-3 ${
        ...
        <h3 className={`text-xs font-extrabold uppercase tracking-wider ${isDark ? 'text-slate-300' : 'text-slate-900'}`}>
          {chartType === 'BAR' ? t("Arus Kas Bulanan") : t("Distribusi Pengeluaran")}
        </h3>
        ...
        <div className="h-56 w-full pt-2">
          <ResponsiveContainer width="100%" height="100%">
            {chartType === 'BAR' ? (
              ...
              <PieChart>
                <Pie
                  data={categoryExpenses.length > 0 ? categoryExpenses : [{ name: t('Lainnya'), value: 100000, color: '#6C4CF5' }]}
                  dataKey="value"
                  nameKey="name"
                  cx="50%"
                  cy="50%"
      ...
      <div className={`p-4 border rounded-[24px] space-y-2 ${
        ...
        <div className="flex items-center space-x-2 text-[#6C4CF5] dark:text-[#A78BFA] text-xs font-bold">
          ...
          <span>{t("Saran Hemat AI FZ Savings")}</span>
        </div>
        <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
          {t("Pengeluaran untuk kategori")} <strong>{t("Makan & Minum")}</strong> {t("merupakan yang terbesar bulan ini. Jika dialokasikan 15% lebih hemat, Anda bisa mempercepat pencapaian target tabungan Anda hingga 2 minggu lebih awal.")}
        </p>
      </div>
    </motion.div>
  );
